...
import { 
  ...
  User, FileClock, Upload, AlertCircle, CheckCircle2, Shield
} from 'lucide-react';
...
const MainLayout = ({ children, activeTab, onTabChange, gymProfile, data }) => {
  ...
  // --- SIDEBAR MENU (FROZEN AS REQUESTED) ---
  const menuItems = [
      ...
      <div className="flex-1 flex flex-col overflow-hidden relative">
        ...
        <header className="h-16 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-4 md:px-6 shrink-0 z-30 relative">
           ...
           <div className="flex-1 max-w-md mx-4 hidden md:block relative group">
              ...
              <input 
                 className="w-full pl-10 pr-4 py-2 bg-slate-100 dark:bg-slate-800 border-none rounded-full text-sm focus:ring-2 focus:ring-[#FF6B35] dark:text-white outline-none placeholder:text-slate-400"
           ...
           <div className="flex items-center gap-2 md:gap-4">
              ...
              {/* 1. Quick Dark Mode Toggle (Icon Only) */}
                  ...
                  <div className="absolute top-12 right-0 w-80 bg-white dark:bg-slate-900 shadow-2xl rounded-xl border dark:border-slate-800 overflow-hidden z-50 animate-in fade-in slide-in-from-top-2">
                     ...
                     <div className="max-h-[400px] overflow-y-auto p-2 space-y-1">
                        {notifications.length === 0 ? (
                           <div className="text-center text-slate-400 text-sm py-6">All caught up!</div>
                        ) : (
                           notifications.map((n) => (
                              <div key={n.id} className={`p-3 rounded-lg flex gap-3 ${n.type === 'alert' ? 'bg-red-50' : 'bg-orange-50'}`}>
              ...
              <div className="relative">
                 <div 
                    ...
                    className="flex items-center gap-3 ml-2 border-l pl-4 border-slate-200 dark:border-slate-700 cursor-pointer group select-none"
                 >
                    <div className="text-right hidden md:block">
                       <p className="text-sm font-bold text-slate-800 dark:text-white leading-none group-hover:text-[#FF6B35] transition-colors">{gymProfile?.name || 'GOKU Fitness'}</p>
                       <p className="text-[10px] text-slate-400 font-bold uppercase mt-0.5 text-right">Admin Access</p>
                    </div>
                    <div className="relative">
                 ...
                 {showProfileMenu && (
                    <div className="absolute top-14 right-0 w-64 bg-white dark:bg-slate-900 shadow-xl rounded-xl border dark:border-slate-800 p-2 z-50 animate-in fade-in slide-in-from-top-2">
                       ...
                       <div className="p-3 border-b dark:border-slate-800 flex items-center gap-3 mb-2">
                          ...
                          <div>
                             <p className="font-bold dark:text-white text-sm">Update Photo</p>
        ...
        {/* --- PAGE CONTENT AREA (Close menus on click) --- */}
        <main 
          className="flex-1 overflow-y-auto p-4 md:p-6 relative scroll-smooth" 
